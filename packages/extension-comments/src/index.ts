...
export const Comment = Mark.create({
  ...
  addCommands: () => ({
    addComment: (...args: unknown[]): Command => {
      const [opts] = args as [
    ...
    resolveComment: (...args: unknown[]): Command => {
    ...
    reopenComment: (...args: unknown[]): Command => {
      const [id] = args as [string]
      return ({ state, dispatch }) => {
        if (!id) return false
        const markType = state.schema.marks['comment']
    ...
    removeComment: (...args: unknown[]): Command => {
      const [id] = args as [string]
      return ({ state, dispatch }) => {
        if (!id) return false
        const markType = state.schema.marks['comment']
    ...
    addCommentReply: (...args: unknown[]): Command => {
